...
def main():
    ...
    if target in content:
        ...
        print("Could not find fuel cost constant")
    ...
    replacement_step = '''        # 1) Execute actions (Velocity Mechanics & Rendezvous Scenario)
        for i in range(2):
    ...
    target_reward = '''        # 4) Calculate Rewards (Formation Flying)
        rewards = [0.0, 0.0]
    ...
    replacement_reward = '''        # 4) Calculate Rewards (Rendezvous & Servicing)
        rewards = [0.0, 0.0]
        # Calculate angular distance (shortest path)
        diff = abs(self.agent_pos[0] - self.agent_pos[1])
        fwd_dist = min(diff, self.num_positions - diff)
